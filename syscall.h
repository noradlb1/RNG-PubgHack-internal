...
{
	PBYTE pFuncAddr = (PBYTE)GetProcAddress(GetModuleHandleA(xorstr_("ntdll.dll")), lpFuncName);
...
void makesyscall<T>::CreateShellSysCall(const uint32_t sysindex)
{
	...
	*PDWORD(ShellCode + 9) = *(PDWORD)GetProcAddress(GetModuleHandleA(xorstr_("ntdll.dll")), xorstr_("Wow64Transition"));
#endif
