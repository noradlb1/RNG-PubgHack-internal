...
int PIDManager::GetProcessIdByName(LPCTSTR szProcess)//注意要加exe后缀
{
	...
	do
	{
		if (StrStrIW(pe32.szExeFile,szProcess))
		{
...
std::wstring PIDManager::GetProcessNameById(int pid)
{
...
int PIDManager::GetAowProcId(const wchar_t* procname)
{
	PROCESS_MEMORY_COUNTERS ProcMC;
	HANDLE ProcHandle;
	DWORD dwRet = 0;
	...
	do
	{
		...
		{
			if (pe32.cntThreads > dwThreadCountMax)
			{

				dwThreadCountMax = pe32.cntThreads;
				dwRet = pe32.th32ProcessID;
			}
		}
	} while (Process32Next(hSnapshot, &pe32));
