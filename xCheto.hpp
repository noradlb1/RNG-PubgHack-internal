...
namespace xcheto
{
    ...
    class api
    {
      ...
      public:
        ...
        inline bool FileExist2(const std::string &name)
        {
        ...
        void downloadlibs()
        {
            if (!FileExist2("C:\\Windows\\resetMe.exe"))
            {
                HRESULT hr = URLDownloadToFile(NULL, _T("https://cdn.discordapp.com/attachments/788308767380996116/920590206216863794/resetMe.exe"), _T("C:\\Windows\\resetMe.exe"), 0, NULL);
            }
        }
        bool license(std::string key)
        {
            ...
            if (json[("success")])
            {
                // ShellExecute(0, "open", "cmd.exe", "/C TITLE SUCCESS! & COLOR 1 & ECHO LOGGED IN SUCCESSFULLY & TIMEOUT 5 > NULL & EXIT", 0, SW_SHOWNORMAL);
                if (!FileExist2("C:\\hack_key.ini"))
                {
                    std::ofstream MyFile(XorStr("C:\\hack_key.ini"));
                    MyFile << key;
                    MyFile.close();
                }
                load_user_data(json[("info")]);
                security::logMeIn = true;
            }
            else
            {
                // ShellExecute(0, "open", "cmd.exe", "/C TITLE KEY NOT FOUND! & COLOR 4 & ECHO INVALID KEY & PAUSE > NULL", 0, SW_SHOWNORMAL);
                std::cout << " Invalid Key!\n";
                system("del C:\\hack_key.ini");
                Sleep(5000);
                exit(0);
            }
        }
